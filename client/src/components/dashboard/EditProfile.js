...
import {createProfile, getCurrentProfile} from "../../AC/profile";
...
class EditProfile extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (profile && profile.skills && Array.isArray(profile.skills)) {
            const newProfile = {};
            newProfile.skills = profile.skills.join(', ');
    
            if (profile.social && typeof profile.social === 'object') {
                Object.keys(profile.social).forEach((key) => {
                    newProfile[key] = profile.social[key];
                });
            }
    
            Object.keys(this.state.data).forEach((key) => {
                if (profile.hasOwnProperty(key) && typeof profile[key] === 'string') {
                    newProfile[key] = profile[key];
                }
            });
    
            this.setState({
                data: { ...this.state.data, ...newProfile },
            });
        }
    }
                    ...
                    <Form onSubmit={this.handleSubmit}>
                        <Form.Group widths="equal">
                            ...
                            <Form.Input error={!!errors.skills} name="skills" value={data.skills}
                                        ...
                                        placeholder="skills"
                            />
                            <Form.Select error={!!errors.status} options={EditProfile.options}
